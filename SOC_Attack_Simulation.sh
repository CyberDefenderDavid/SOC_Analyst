...
# Tools to check and install is not intalled 
tools=("git" "nmap" "hydra" "curl" "hping3" "seclists")  
...
install_tools() {
    for tool in "${tools[@]}"; do
        ...
        if [ "$tool" == "seclists" ]; then
			...
			#will install if item not installed
    ...
    # Check for ntpd command and install ntp package if not found
    if ! command -v ntpd > /dev/null 2>&1; then
        echo "ntpd command not found. Attempting to install ntp..."
        apt-get install ntp -y
        if [ $? -eq 0 ]; then
            echo "ntp package installed successfully."
        else
            echo "Failed to install ntp package. Exiting."
            exit 1
        fi
    else
        echo "ntpd command is available. Skipping installation."
    fi
}
...
get_ip() {
    ...
    while true; do
        ...
        if [ -z "$ip" ]; then
            ...
            echo "Targeting user-specified IP: $ip" # echo the $ip
            break
        else
...
hydra_custom_service_and_port() {
    ...
    if [ -z "$service" ]; then
        ...
        echo "Custom service $service on port." # echo the $service and $port
...
confirm_and_execute_hydra() {  
    ...
    if [[ "$confirm" =~ ^[Yy]$ ]]; then
		...
		echo "Hydra attack completed. Exiting script."
        exit 
    else
...
sql_perform_attack() {
    ...
    # local for the responces, timestamp and log_message
    ...
    case $response in
        404)
            response_message="$timestamp - Resource Not Found: The path does not exist." #404 means sql attack launched but has sanitisation & validation to prevent errors
            ;;
        200)
            response_message="$timestamp - Success: The request was successfully processed." #200 means sql attack worked on the sql.
            ;;
        *)
...
# sets up the parameters for the SQL injection attack
run_sql() {
    get_ip
